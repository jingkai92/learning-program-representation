...
class GCNSummarizeModel(nn.Module):
    def __init__(self, config):
        super(GCNSummarizeModel, self).__init__()
        ...
        # GCN Layers
        self.gcn_layers = nn.ModuleList([GCNLayer(config) for _ in range(self.graph_config['layers'])])

        # Transformation Layers
        self.g_repr = nn.Linear(self.out_dim, self.out_dim)
        self.node_repr = nn.Linear(self.out_dim, self.out_dim)
        self.hid_fc = nn.Linear(self.out_dim, self.out_dim)
        self.cell_fc = nn.Linear(self.out_dim, self.out_dim)

        # Decoder Params
        self.lstm_dims = self.config.lstm['dims']
        self.lstm_dropout = self.config.lstm['dropout']
        self.attention = self.config.lstm['attention']
    ...
    def greedy_decode(self, batch_size, tgt_len, hidden, cell, encoder_outputs, tgt=None,
                      loss_fn=None, running_mode=""):
        loss = 0
        ...
        for t in range(1, tgt_len):
            ...
            if self.attention:
                ...
                output, attn, hidden, cell = self.decoder(embed_itenser, hidden, cell, None)
