...
class PairwiseGATModel(nn.Module):
    def __init__(self, config):
        super(PairwiseGATModel, self).__init__()
        ...
        # GCN Layers
        self.gat_layers_one = nn.ModuleList([GATLayer(config) for _ in range(self.graph_config['layers'])])
        self.gat_layers_two = nn.ModuleList([GATLayer(config) for _ in range(self.graph_config['layers'])])
    ...
    def forward(self, batch_dict, running_mode, loss_fn):
        ...
        fnone_mean_feats = self.gcn_prop(batch_dict['graphs_one'], self.gat_layers_one)
        fntwo_mean_feats = self.gcn_prop(batch_dict['graphs_two'], self.gat_layers_two)
